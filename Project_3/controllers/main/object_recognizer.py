...
def ransac(matches, threshold, iters):
    ...
    for i in range(iters):
        ...
        if num_inliers > num_best_inliers:
            ...
            best_H = H.copy()
    print("inliers/matches: {}/{}".format(num_best_inliers, len(matches))) # for debugging
...
class ObjectRecognizer:
    ...
    def compute_distance_and_angle(self, x_min, x_max, scene_image):
        ...
        # print("Distance: ", distance, "Angle: ", angle) # for debugging
        return distance, angle
    def detect_objects(self, scene_image: np.ndarray) -> np.ndarray:
        ...
        if len(good) < self.__k:
            return None
        matches_param = np.array([[kp_target[m.queryIdx].pt[0], kp_target[m.queryIdx].pt[1], kp_scene[m.trainIdx].pt[0], kp_scene[m.trainIdx].pt[1]] for m in good])
        _, best_H = ransac(matches_param, 10, 1000)
        x_min, y_min, x_max, y_max = find_position(best_H, self.__target_image_gray, scene_image)
