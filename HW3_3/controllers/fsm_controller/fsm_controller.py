    ...
    def wander(self):
        ...
        if self.wandering_counter >= Controller.MAX_WANDERING_COUNTER:
            rand = np.random.uniform(
                -Controller.MAX_SPEED / 4, Controller.MAX_SPEED / 4
            )
            self.left_speed = np.clip(
                self.left_speed + rand, 0, Controller.MAX_SPEED / 2
            )
            self.right_speed = np.clip(
                self.right_speed - rand, 0, Controller.MAX_SPEED / 2
            )
            self.wandering_counter = 0
        else:
    ...
    def run(self):
        ...
        while self.step(self.timeStep) != -1:
            if self.state == RobotState.WANDER_FOR_CAN:
                ...
                """
                see_target, left_speed, right_speed = self.center_color(
                    self.current_target
                )
                if not see_target:
                    self.state = RobotState.WANDER_FOR_CAN
